/**
 * Tipos e cálculos de data das fases — sem acesso a banco, sem
 * `server-only`: o countdown ao vivo roda no cliente e precisa derivar o
 * alvo da contagem a partir do status que chega do servidor. O módulo
 * `phases.ts` (server-only) concentra a LEITURA; aqui fica só aritmética
 * de data, que é idêntica nos dois lados.
 */
